...
$BootConfigDockerLabel.location       = New-Object System.Drawing.Point(490,260)
...
$BootConfigDocker.location            = New-Object System.Drawing.Point(700,257)
...
#--------------[one-time-script.service]-------------------
  ...
  ExecStart=/usr/local/bin/docker_setup.sh || true
'@
...
passwd --expire pi
$(if ($BootConfigDocker.Checked){"passwd -l pi"})
raspi-config nonint do_boot_behaviour B3
# Remove autostart of welcome to raspberry pi
rm /etc/xdg/autostart/piwiz.desktop
# install docker
$(if ($BootConfigDocker.Checked){"sed -i `'s/\r$//`' /boot/docker_setup.sh"})
$(if ($BootConfigDocker.Checked){"sed -i 's/\r$//' /boot/one_time_script.service"})


$(if ($BootConfigDocker.Checked){"mv /boot/one_time_script.service /lib/systemd/system/"})
$(if ($BootConfigDocker.Checked){"ln -s /lib/systemd/system/one_time_script.service /etc/systemd/system/multi-user.target.wants/"})
$(if ($BootConfigDocker.Checked){"ln -s /lib/systemd/system/systemd-time-wait-sync.service /etc/systemd/system/multi-user.target.wants/"})
$(if ($BootConfigDocker.Checked){"mv /boot/docker_setup.sh /usr/local/bin/docker_setup.sh"})
$(if ($BootConfigDocker.Checked){"chmod +x /usr/local/bin/docker_setup.sh"})
...
#runuser - pi -c 'export XDG_RUNTIME_DIR=/run/user/$UID && dockerd-rootless-setuptool.sh install --force' | systemd-cat -t docker -p warning

systemctl disable one_time_script.service | systemd-cat -t docker -p warning

'@
$(if ($BootConfigDocker.Checked){Set-Content -Path $dockerfilename -Value $dockerfile})
